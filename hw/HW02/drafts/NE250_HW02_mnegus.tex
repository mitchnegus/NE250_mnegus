...
The general one-group diffusion equation is 
$$ \frac{1}{v}\frac{\p \phi\rt}{\p t} = S\rt - \Sigma_{a,1}(\pos) \phi\rt + \nabla \cdot D(\pos) \nabla \phi\rt .$$
If we assume that our reflected system is steady state, then $\frac{\p \phi\rt}{\p t} = 0$. Also, we assume that our system is homogenous in each region (slab and reflector) so that $\Sigma_{a}(\pos) = \Sigma_{a,\text{X}}$ and $D(\pos) = D_{\text{X}}$ for each region X in the system. Then, we rearrange to find
$$ -  D_{\text{X}} \nabla^2 \phi(\pos) + \Sigma_{a,\text{X}} \phi(\pos) = S(\pos) .$$
Where we are considering a slab geometry, the Laplacian of the flux simplifies to a second derivative, and $\pos$ is replaced by $x$.
$$ \frac{d\phi(\pos)}{dx} - \frac{\Sigma_{a,\text{X}}}{D_{\text{X}}}\phi(\pos)  = -\frac{S(\pos)}{D_{\text{X}} } .$$
We let $L_{\text{X}} = \sqrt{\frac{\Sigma_{a,\text{X}}}{D_{\text{X}}}}$, and so 
$$ \frac{d^2\phi(x)}{dx^2} - \frac{1}{L_{\text{X}}^2}\phi(x)  = -\frac{S(x)}{D_{\text{X}} } .$$

Noting the symmetry of our system (a slab of width $2b$ with the midplane at the origin, bounded by a reflector of width $a$ on each side), we can give the one-speed diffusion equation in the slab as
$$ \frac{d^2\phi(x)}{dx^2} - \frac{1}{L_{\text{S}}^2}\phi(x)  = -\frac{S_0}{D_{\text{S}} } $$
and in the reflector as
$$ \frac{d^2\phi(x)}{dx^2} - \frac{1}{L_{\text{R}}^2}\phi(x)  = 0 .$$
The boundary conditions on our system are that 
\begin{multicols}{2}
$$ \phi_S(b) = \phi_R(b) $$
$$ \cur_S(b) = \cur_R(b) $$
